...
THEN DO;
     ...
     *** MCAID = 0 IF NO, DOES NOT HAVE MEDICAID OR PUBLIC ASSISTANCE OR UNKNOWN ON SUCH COVERAGE ;
     IF MEDICAID IN (1,2) OR OTHERPUB IN (1) OR CHIP IN (1)
     THEN MCAID = 1;
     ELSE IF (MEDICAID > 3 OR MEDICAID <= 0) OR (CHIP > 2 OR CHIP <= 0) OR (OTHERPUB > 2 OR OTHERPUB <= 0) 
          THEN MCAID = 9;
          ELSE MCAID = 2;
     END;
